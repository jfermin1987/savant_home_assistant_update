...
when 'result'
  if msg['success']
    ...
    # Home Assistant requires monotonically increasing ids on a given WS session.
      # In some HA restart/reconnect edge-cases, the server can reject the first few
      # requests with id_reuse. Bump our counter forward to recover automatically.
      if msg.dig('error', 'code') == 'id_reuse'
        bump = 1000
        @id_mutex.synchronize { @next_id += bump }
        log(:warn, :id_reuse_bump, bump, :next_id, @next_id)
      end
