...
"
  ...
  # ---------------------------
  EM_BOOT_MUTEX = Mutex.new unless const_defined?(:EM_BOOT_MUTEX)

  def ensure_em_running
    return if EM.reactor_running?

    self.class::EM_BOOT_MUTEX.synchronize do
      return if EM.reactor_running?
      # Start EM reactor once. If the thread dies for any reason, start it again.
      @@em_thread = nil unless defined?(@@em_thread) && @@em_thread&.alive?
      @@em_thread ||= Thread.new do
        begin
          EM.run
        rescue => e
          p([:error, :em_run_crash, e.class.to_s, e.message])
        end
      end
    end

    # Small spin-wait for reactor
    100.times do
  ...
  def em_schedule(&blk)
    ...
    p([:error, :em_schedule_failed, e.class.to_s, e.message])
  end
  ...
  def em_add_timer(seconds, &blk)
    ...
    p([:error, :em_add_timer_failed, e.class.to_s, e.message])
  end
  ...
  def em_add_periodic_timer(seconds, &blk)
    ...
    p([:error, :em_add_periodic_timer_failed, e.class.to_s, e.message])
  end
