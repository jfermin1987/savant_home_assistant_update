...
"
  ...
  def start_savant_heartbeat
    ...
    if EM.reactor_running?
      EM.schedule do
        ...
        # this 'hello' nudges it (and helps detect half-open socket)
        send_to_savant('hello,proxy=ha_savant,proto=1')
      end
    else
      # Fallback (should rarely happen): keepalive loop without EM timers
      Thread.new do
        loop do
          break if @shutdown
          sleep SAVANT_HELLO_INTERVAL
          send_to_savant('hello,proxy=ha_savant,proto=1')
        end
      end
    end
  ...
  # ---------------------------
  def em_schedule(&blk)
    return blk.call unless EM.reactor_running?
    EM.schedule(&blk)
  rescue => e
  ...
  def em_add_timer(delay, &blk)
    em_schedule { em_add_timer(delay, &blk) }
  end

  def em_add_periodic_timer(interval, &blk)
    em_schedule { em_add_periodic_timer(interval, &blk) }
  end
  ...
  def ensure_em_running
    ...
    # Ensure EM reactor starts exactly once, even if multiple Savant clients connect.
    @@em_mutex ||= Mutex.new
    @@em_thread ||= nil
    ...
    @@em_mutex.synchronize do
      ...
      # Wait briefly for reactor to become ready
      timeout = Time.now + 3
      until EM.reactor_running?
        raise 'EventMachine reactor failed to start' if Time.now > timeout
        sleep 0.02
      end
    end
  end
