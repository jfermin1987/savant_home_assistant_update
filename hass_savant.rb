...
"
  ...
  def start_savant_heartbeat
    # Periodically nudge Savant so it sends config after reboots and to detect half-open sockets.
    ensure_em_running
  ...
  # ---------------------------
  # EventMachine helpers (thread-safe)
  # ---------------------------
  def em_schedule(&blk)
    # This add-on runs EM.run in the main thread (see bottom of file).
    # Client handlers run in separate threads, so we must schedule work onto the EM reactor thread.
    # If the reactor is not ready yet, wait briefly instead of trying to start EM again.
    timeout = Time.now + 3
    until EM.reactor_running?
      raise 'EventMachine reactor not running' if Time.now > timeout
      sleep 0.02
    end
    EM.schedule(&blk)
  rescue => e
    p([:error, :em_schedule_failed, e.message])
  end

  def em_add_timer(delay, &blk)
    em_schedule { EM.add_timer(delay, &blk) }
  end

  def em_add_periodic_timer(interval, &blk)
    em_schedule { EM.add_periodic_timer(interval, &blk) }
  end
 ...
 (auto reconnect + queue)
  ...
  def ensure_em_running
    ...
    # Do NOT start a second EM loop; just wait until the main reactor is running.
    timeout = Time.now + 3
    until EM.reactor_running?
      raise 'EventMachine reactor not running' if Time.now > timeout
      sleep 0.02
    ...
    p([:error, :em_not_ready, e.message])
  end

  def connect_websocket
    ensure_em_running
    em_schedule { start_ws }
  end
  ...
  def start_ws
    ...
    # Faye::WebSocket requires EventMachine reactor; create the client on the EM thread.
    em_schedule do
      begin
      em_schedule { @hass_ws&.close }
      rescue
      end
  ...
  def schedule_reconnect
    ...
    if EM.reactor_running?
      ...
      Thread.new { sleep(delay); start_ws }
    end
  end
  ...
  def start_ha_ping
    ...
    em_add_periodic_timer(HA_PING_INTERVAL) do
      next if @shutdown
      send_json(type: 'ping')
    end
  end
  ...
  loop do
    ...
    Thread.new do
      begin
        ...
        # Keep this client handler alive while the socket is active
        loop do
          break if hass_instance.instance_variable_get(:@shutdown)
          sleep 1
        end
      rescue => e
...
EM.run do
  Thread.new { start_tcp_server(WS_URL, WS_TOKEN, TCP_PORT) }
end
