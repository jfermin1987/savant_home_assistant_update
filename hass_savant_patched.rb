...
class SavantConn < EM::Connection
  ...
  # Format values before sending to Savant.
  # Savant XML profiles often expect integers for temperatures (e.g., "23" not "23.0").
  ...
  def fmt_value(v)
    ...
    if v.is_a?(Numeric)
      f = v.to_f
      # If it's effectively an integer (23.0), send without decimals.
      return f.round.to_i.to_s if (f - f.round).abs < 1e-6

      # Otherwise, send a compact decimal without trailing zeros.
      s = format('%.3f', f)
      return s.sub(/0+$/, '').sub(/\.$/, '')
    end
  ...
  def send_update(entity_id, key, value)
    send_data("#{entity_id}_#{key}===#{fmt_value(value)}\n")
  rescue StandardError => e
