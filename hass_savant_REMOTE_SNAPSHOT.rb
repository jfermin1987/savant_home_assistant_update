...
class SavantConn < EM::Connection
  ...
  def send_update(entity_id, key, value)
    log(:debug, :to_savant, entity_id, key, value) if ENV['DEBUG_OUT'] == '1'
...
def ensure_ha_subscribed(entity_ids)
  ...
  def forward_entity_to_client(client, entity_id, packed, filter)
    ...
    # HVAC normalization (Remote HVAC UI is strict; always emit a coherent snapshot)
    if entity_id.start_with?('climate.')
      # HA's climate "state" is often the current hvac_mode (e.g., "cool", "off").
      # Some integrations also expose an explicit hvac_mode attribute. Prefer attribute, fall back to state.
      hvac_mode = (attrs['hvac_mode'] || state)
      hvac_action = attrs['hvac_action']
      preset_mode = attrs['preset_mode']

      # Target setpoint: HA uses "temperature" for single setpoint. Some devices expose alternates.
      ...
      # Always send hvac_mode if we have *any* value (Remote needs this for enablement)
      client.send_update(entity_id, 'hvac_mode', hvac_mode) unless hvac_mode.nil?

      # hvac_action is optional in HA; if missing but mode is not off, default to idle so Savant UI can enable
      if hvac_action.nil? && !hvac_mode.to_s.empty? && hvac_mode.to_s != 'off'
        hvac_action = 'idle'
      end
      client.send_update(entity_id, 'hvac_action', hvac_action) unless hvac_action.nil?

      client.send_update(entity_id, 'preset_mode', preset_mode) unless preset_mode.nil?
