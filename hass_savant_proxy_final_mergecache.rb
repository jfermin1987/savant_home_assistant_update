...
class HaWs
  ...
  when 'result'
    ...
    # If this was a get_states call we triggered, convert the full state list
    # into a snapshot compatible with our subscribe_entities handler.
    if @pending_get_states && @pending_get_states.delete(msg['id'])
      states = msg['result']
      if states.is_a?(Array)
        ...
        states.each do |st|
          ...
          eid = st['entity_id']
          next if eid.to_s.empty?
          snapshot[eid] = { 's' => st['state'], 'a' => (st['attributes'] || {}) }
        end
        @on_event&.call({ 'type' => 'event', 'event' => { 'a' => snapshot, 'c' => {} } })
      end
    end
  end
rescue StandardError => e
...
def ensure_ha_subscribed(entity_ids)
  ...
  def forward_entity(entity_id, packed)
    ...
    # IMPORTANT: HA "subscribe_entities" deltas often send *partial* payloads (only changed keys).
    # If we simply overwrite the cache with the delta, we lose previously-known keys like:
    #   - packed['s'] (hvac_mode/state)
    #   - packed['a']['hvac_action'] (cooling/heating/idle)
    # which causes newly-connected Savant HVAC UIs to miss mode/action until something changes again.
    @entity_cache[entity_id] = merge_packed(@entity_cache[entity_id], packed)
  ...
  def merge_packed(existing, incoming)
    # packed format:
    #   { 's' => <state>, 'a' => { <attr_key> => <attr_val>, ... } }
    # incoming may omit 's' and/or 'a' when it is a delta patch.
    return incoming unless existing.is_a?(Hash)
    return existing unless incoming.is_a?(Hash)

    merged = existing.dup

    if incoming.key?('s') && !incoming['s'].nil?
      merged['s'] = incoming['s']
    end

    if incoming['a'].is_a?(Hash)
      merged['a'] = (merged['a'].is_a?(Hash) ? merged['a'].dup : {})
      incoming['a'].each { |k, v| merged['a'][k] = v }
    end
