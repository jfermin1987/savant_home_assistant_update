...
class SavantConn < EM::Connection
  ...
  def handle_line(line)
    ...
    when 'state_filter'
      ...
      # Some Savant HVAC remotes never send subscribe_entity.
      # They only set a state_filter, then expect the controller to "just work".
      # If we see a climate-style filter, automatically enable subscribe_all_events
      # for this client so it receives state updates needed to unlock +/- UI.
      if (@filter & %w[temperature current_temperature hvac_action hvac_mode preset_mode]).any?
        @subscribe_all = true
        @proxy.save_subs(current_identity, subscribe_all: true)
        log(:info, :auto_subscribe_all_enabled, current_identity)
      end
...
def ensure_ha_subscribed(entity_ids)
  ...
  def handle_action(cmd, args)
    ...
    when 'climate_set_hvac_mode'
      ...
      optimistic_climate_mode(entity, mode)
    when 'climate_set_fan_mode'
      entity = args[0]
      fan = (args[1] || 'auto').to_s
      service_call('climate', 'set_fan_mode', entity, { fan_mode: fan })
      optimistic_climate_fan_mode(entity, fan)
    when 'ha_refresh'
      # Force an immediate snapshot refresh (used by Savant Remote on HVAC UI open)
      entity = args[0].to_s
      # Replay last cached pack first so UI can enable immediately
      if !entity.empty? && @entity_cache[entity]
        forward_entity(entity, @entity_cache[entity])
      end
      request_state_refresh
...
# ---- Optimistic UI updates for Savant Remote ----
# Savant remotes often require immediate feedback to enable setpoint controls.
# HA state changes can arrive slightly later via websocket events, so we
# immediately update our cache and forward a best-effort state pack.
def optimistic_climate_mode(entity_id, mode)
  mode = mode.to_s.strip
  return if mode.empty?

  packed = (@entity_cache[entity_id] || { 's' => mode, 'a' => {} })
  packed = { 's' => packed['s'], 'a' => (packed['a'] || {}).dup } # shallow dup
  packed['s'] = mode
  packed['a']['hvac_mode'] = mode
  @entity_cache[entity_id] = packed
  forward_entity(entity_id, packed)
  # Force a near-term refresh to reconcile with HA truth.
  EM.add_timer(0.25) { request_state_refresh }
end

def optimistic_climate_temp(entity_id, temperature)
  return if entity_id.to_s.strip.empty?
  packed = (@entity_cache[entity_id] || { 's' => nil, 'a' => {} })
  packed = { 's' => packed['s'], 'a' => (packed['a'] || {}).dup }
  packed['a']['temperature'] = temperature
  @entity_cache[entity_id] = packed
  forward_entity(entity_id, packed)
  EM.add_timer(0.25) { request_state_refresh }
end

def optimistic_climate_fan_mode(entity_id, fan_mode)
  fan_mode = fan_mode.to_s.strip
  return if fan_mode.empty?
  packed = (@entity_cache[entity_id] || { 's' => nil, 'a' => {} })
  packed = { 's' => packed['s'], 'a' => (packed['a'] || {}).dup }
  packed['a']['fan_mode'] = fan_mode
  @entity_cache[entity_id] = packed
  forward_entity(entity_id, packed)
  EM.add_timer(0.25) { request_state_refresh }
end
  ...
  def forward_entity_to_client(client, entity_id, packed, filter)
    ...
    # Format values in a Savant-friendly way:
    # - Numeric values like 23.0 -> "23" (avoids integer parsers rejecting floats)
    # - Preserve non-integer decimals like 23.5 -> "23.5" (no trailing zeros)
    fmt = lambda do |v|
      return '' if v.nil?
      ...
      if v.is_a?(Numeric)
        ...
        if (f - f.round).abs < 1e-6
          return f.round.to_i.to_s
        end
        return ('%.3f' % f).sub(/0+$/, '').sub(/\.$/, '')
      end
      v.to_s
    end
    ...
    Array(filter).each do |k|
      case k
      when 'state'
        client.send_update(entity_id, 'state', fmt.call(state)) unless state.nil?
      when 'attributes'
        client.send_update(entity_id, 'attributes', JSON.generate(attrs))
      else
        v = attrs[k]
        client.send_update(entity_id, k, fmt.call(v)) unless v.nil?
      end
    end

    # HVAC UI helpers (some XMLs bind to these explicitly)
    if entity_id.start_with?('climate.')
      # HA: mode usually comes as packed['s'] (entity state), action lives in attributes.
      ha_mode = (attrs['hvac_mode'] || state).to_s
      ha_action = attrs['hvac_action'].to_s

      client.send_update(entity_id, 'hvac_mode', fmt.call(ha_mode)) unless ha_mode.empty?
      client.send_update(entity_id, 'hvac_action', fmt.call(ha_action)) unless ha_action.empty?

      # ------------------------------
      # Vera-style "state pack" for Savant Remotes
      # Many Savant remotes only enable setpoint +/- when these are present:
      #   ModeTarget / ModeStatus (Off, CoolOn, HeatOn, AutoChangeOver)
      #   ModeState  (Cooling, Heating, Idle, Off)
      #   CurrentSetpoint / SetpointTarget (and optionally Cool/Heat setpoints)
      # ------------------------------

      vera_mode = case ha_mode.downcase
                  when 'off' then 'Off'
                  when 'cool' then 'CoolOn'
                  when 'heat' then 'HeatOn'
                  when 'auto', 'heat_cool' then 'AutoChangeOver'
                  when 'fan_only' then 'FanOn'
                  when 'dry' then 'Dry'
                  else ha_mode
                  end

      vera_state = case ha_action.downcase
                   when 'cooling' then 'Cooling'
                   when 'heating' then 'Heating'
                   when 'idle' then 'Idle'
                   when 'off' then 'Off'
                   when ''
                     # If HA doesn't report action, fall back to mode.
                     ha_mode.downcase == 'off' ? 'Off' : 'Idle'
                   else ha_action
                   end
      ...
      client.send_update(entity_id, 'mode_target', vera_mode)
      client.send_update(entity_id, 'mode_status', vera_mode)
      client.send_update(entity_id, 'mode_state', vera_state)

      unless target.nil?
        t = fmt.call(target)
        client.send_update(entity_id, 'setpoint_target', t)
        client.send_update(entity_id, 'current_setpoint', t)
        client.send_update(entity_id, 'cool_setpoint', t)
        client.send_update(entity_id, 'heat_setpoint', t)
      end

      client.send_update(entity_id, 'current_temp', fmt.call(current)) unless current.nil?
    end
  end
end
